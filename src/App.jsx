...
const chapterKnowledgeBase = [
  {
    ...
    content:
      'Chương 4 - Dòng điện. Mạch điện gồm các nội dung: cường độ dòng điện; điện trở và định luật Ohm; nguồn điện; năng lượng điện và công suất điện; thực hành đo suất điện động và điện trở trong của pin điện hoá. Bối cảnh mở đầu: xe điện phát triển nhờ pin có khả năng dự trữ năng lượng, sau khi sạc có thể duy trì dòng điện trong mạch kín.',
  },
  {
...
const featureContent = {
  overview: {
    ...
    body: 'Chương 4 gồm 5 bài học chính theo nội dung trong hình: Bài 22, Bài 23, Bài 24, Bài 25 và Bài 26.',
  ...
  formulas: {
    ...
    body: 'Công thức trọng tâm theo Chương 4: I = Δq / Δt, I = Ne / t, I = Snve, R = U / I, I = U / R, R = ρ.l / S, ξ = A / q, I = ξ / (R + r), U = ξ - Ir, Q = mcΔt, W = UIt, P = W / t = UI, Q = I²Rt, 1 kW.h = 3,6.10^6 J.',
  ...
  progress: {
    ...
    body: 'Bạn đang học Chương 4 với 5 bài: Bài 22, Bài 23, Bài 24, Bài 25 và Bài 26 thực hành.',
    actions: [],
  },
  achievements: {
...
const isAnswerRevealRequest = (question) => {
  const normalizedQuestion = normalizeText(question)
...
Logic học tập do code quyết định. Bạn chỉ sinh nội dung đúng theo NHIỆM VỤ của lượt này, không tự đổi sang kiểu trả lời khác.

KIỂM TRA TRƯỚC KHI GỬI
Trước khi gửi phản hồi, tự kiểm tra: có đúng nhiệm vụ không; có lạc sang bài khác không; có dùng kiến thức sai không; có đưa đáp án khi đang ở chế độ gợi ý không; học sinh THPT đọc có hiểu ngay không.
...
ƯU TIÊN NGUỒN:
Luôn ưu tiên nội dung chuẩn trong hệ thống/website được cung cấp. Chỉ dùng kiến thức bên ngoài khi câu hỏi vượt ngoài nội dung hệ thống hoặc thật sự cần mở rộng nâng cao; nếu dùng ngoài hệ thống thì nói ngắn gọn rằng đó là phần mở rộng.

KHÔNG ĐƯỢC LÀM:
Không tạo bài tập mới nếu học sinh không yêu cầu. Không làm thay toàn bộ bài tập khi nhiệm vụ là gợi ý. Không trả lời lan man ngoài phạm vi bài học. Không dùng ngôn ngữ quá hàn lâm. Không khuyến khích học sinh học thuộc máy móc.
...
Ngắn gọn, dễ hiểu, chính xác, giống giáo viên đang hỗ trợ học sinh. Ưu tiên hiểu bản chất vật lí. Không trả lời dài dòng. Không dùng thuật ngữ khó nếu có thể diễn đạt đơn giản hơn.`
  const modeInstruction =
    mode === 'assessment'
      ? `NHIỆM VỤ: Chấm câu trả lời của học sinh so với câu hỏi gốc.
Câu hỏi gốc: "${safeOriginalQuestion}"
Câu trả lời học sinh: "${studentAnswer || question}"
Hãy đối chiếu với kiến thức chuẩn trong website trước. Chỉ trả về đúng một dòng theo mẫu: "Điểm: NN%". Không giải thích, không đưa đáp án, không thêm chữ khác.`
      : mode === 'final'
        ? `NHIỆM VỤ: Hiển thị đáp án chuẩn và chốt kiến thức vì code đã cho phép hiện đáp án.
Câu hỏi gốc: "${safeOriginalQuestion}"
${studentAnswer ? `Câu trả lời học sinh: "${studentAnswer}"` : 'Học sinh chưa đưa câu trả lời để đối chiếu.'}
Bắt buộc trình bày đúng 4 mục sau, ngắn gọn:
- Đáp án chuẩn:
- Em đã đúng ở:
- Em cần sửa/bổ sung:
- Ghi nhớ:
  ...
  const response = await fetch("/api/ai/chat", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: 'system',
          content: `Ngữ cảnh học tập: học sinh đang ở Chương 4 - Dòng điện. Mạch điện. Chủ đề đang chọn: ${topic.label}. Điểm nắm vững gần đúng: ${memory.mastery[topic.id]?.score ?? 0}/100. Các phần nên luyện thêm: ${weakTopics}.`,
        },
        {
          role: 'system',
          content: `Hệ thống kiến thức cố định của Chương 4. Khi trả lời, phải ưu tiên bám sát hệ thống kiến thức này hơn kiến thức chung; nếu cần suy luận ngoài phạm vi thì nói rõ.\n\n${lessonKnowledgeContext}`,
        },
        {
          role: 'system',
    ...
    <div className="repair-game">
      <header className="repair-hero">
        <div>
          ...
          <h2>Sửa chữa hệ thống điện Chương 4</h2>
...
function App() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      text:
        'Xin chào! Mình là trợ lí học tập Vật lí 11. Khi bạn hỏi kiến thức mới, mình sẽ gợi ý trước để bạn tự trả lời; nếu bạn trả lời đạt, mình sẽ chốt đáp án chuẩn và phần cần bổ sung.',
    },
  ])
    ...
    setMessages([
      {
        ...
        text:
          savedMemory
            ? 'Xin chào! Thành tích và tiến độ học trước đó của bạn đã được tải lại. Khi bạn hỏi kiến thức mới, mình sẽ gợi ý trước để bạn tự trả lời; sau đó mình chấm, gợi ý tiếp hoặc chốt đáp án chuẩn.'
            : 'Xin chào! Tài khoản mới đã bắt đầu từ 0. Khi bạn hỏi kiến thức mới, mình sẽ gợi ý trước để bạn tự trả lời; sau đó mình chấm, gợi ý tiếp hoặc chốt đáp án chuẩn.',
      },
    ])
  ...
  const startAiExercise = async (topicId = selectedTopicId) => {
    ...
    try {
      const aiText = await requestAiResponse({
        ...
        mode: 'hint',
        originalQuestion: topic.exercise,
      })
      ...
      setMessages((currentMessages) => [
        ...currentMessages,
        { role: 'assistant', text: cleanAiDisplayText(aiText) },
      ])
      setPendingGuidance({
        question: topic.exercise,
        topicId: topic.id,
  ...
  const handleAiSubmit = async (event) => {
    ...
    const wantsImmediateAnswer = isAnswerRevealRequest(question)
    const wantsMoreGuidance = Boolean(pendingGuidance && isMoreGuidanceRequest(question) && !wantsImmediateAnswer)
    const startsNewQuestion = Boolean(
      pendingGuidance && isLikelyNewQuestion(question) && !wantsMoreGuidance && !wantsImmediateAnswer,
    )
    ...
    const currentGuidanceAttempt = pendingGuidance && !startsNewQuestion ? pendingGuidance.attempts || 1 : 0
    ...
    try {
      let aiText = ''
      let nextPendingGuidance = null
      let memoryDelta = 4

      if (pendingGuidance && !startsNewQuestion && wantsImmediateAnswer) {
        aiText = await requestAiResponse({
          question,
          guidanceAttempt: currentGuidanceAttempt,
          topic: conversationTopic,
          memory: activeMemory,
          previousMessages,
          mode: 'final',
          originalQuestion,
          studentAnswer: pendingGuidance.lastAnswer || '',
        })
        memoryDelta = 1
      } else if (pendingGuidance && !startsNewQuestion && wantsMoreGuidance) {
        const nextAttempt = currentGuidanceAttempt + 1

        if (nextAttempt > MAX_GUIDANCE_ATTEMPTS) {
          aiText = await requestAiResponse({
            question,
            guidanceAttempt: currentGuidanceAttempt,
            topic: conversationTopic,
          ...
          nextPendingGuidance = {
            ...
            lastAnswer: pendingGuidance.lastAnswer || '',
          }
          memoryDelta = 0
        }
      } else if (pendingGuidance && !startsNewQuestion) {
        const assessmentText = await requestAiResponse({
          question,
          guidanceAttempt: currentGuidanceAttempt,
          topic: conversationTopic,
          memory: activeMemory,
        ...
        const assessment = parseAiAssessment(assessmentText)

        if (assessment.score === null) {
          throw new Error('DeepSeek không trả về điểm đánh giá hợp lệ')
        }

        if (assessment.score >= PASSING_ANSWER_SCORE || currentGuidanceAttempt >= MAX_GUIDANCE_ATTEMPTS) {
          aiText = await requestAiResponse({
            question: originalQuestion,
            guidanceAttempt: currentGuidanceAttempt,
            topic: conversationTopic,
          ...
          memoryDelta = assessment.score >= PASSING_ANSWER_SCORE ? 12 : -4
        } else {
          const nextAttempt = currentGuidanceAttempt + 1
          aiText = await requestAiResponse({
            question: originalQuestion,
      ...
      setMemory((currentMemory) => {
        const baseMemory = startsNewQuestion ? { ...currentMemory, pendingQuiz: null } : currentMemory
        const rememberedMemory = rememberTopic(baseMemory, conversationTopic.id, memoryDelta)

        return nextPendingGuidance ? rememberedMemory : { ...rememberedMemory, pendingQuiz: null }
      })
      setPendingGuidance(nextPendingGuidance)
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Không gọi được AI endpoint')
    } finally {
      setIsAiLoading(false)
    ...
    <div className={`app-shell ${isSidebarOpen ? 'app-shell--sidebar-open' : 'app-shell--sidebar-closed'}`}>
      ...
      <div className="workspace">
        <header className="topbar">
          ...
          <h1>Chương 4: Dòng điện. Mạch điện</h1>
            ...
            <section className="panel ai-panel">
              ...
              <div className="ai-intro">
                ...
                <div>
                  ...
                  <ul>
                    <li>Gợi ý từng cấp khi gặp kiến thức mới</li>
                    <li>Đánh giá câu trả lời theo mốc 60%</li>
                    <li>Chốt đáp án chuẩn khi bạn đạt hoặc yêu cầu hiện đáp án</li>
                    <li>Ưu tiên kiến thức đã có trong website</li>
                  </ul>
                </div>
              </div>
